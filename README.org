...
* Modifying & Rebuilding Guide

| *FLAKE*   | *local build*                | *remote build*                         |
|-----------+------------------------------+----------------------------------------|
| pmp       | nix run . -- pmp             | ./bin/remote-build $DARWIN_HOST pmp    |
| pmpcmt    | nix run . -- pmpcmt          | ./bin/remote-build $DARWIN_HOST pmpcmt |
| vm        | nix run . -- vm              | ./bin/remote-build $NIXOS_HOST vm      |
| $user     | N/A                          | ./bin/remote-build $LINUX_HOST $user   |
| ubuntu    | N/A                          | ./bin/remote-build $LINUX_HOST ubuntu  |
| testvm    | ./bin/vm-build testvm 111    | -                                      |
| minimal   | ./bin/vm-build minimal 112   | -                                      |
| waylandvm | ./bin/vm-build waylandvm 113 | -                                      |
|           |                              |                                        |
...
*** Creating secrets
An example setup of secrets is available in the ~secrets_example/~ directory. This repo uses ~secrets~ as a flake. Rather than editing the example directory, it is recommended that you copy it to ~$HOME/.config/nix-secrets~

1. ~cp -r secrets_example ~/.config/nix-secrets~
2. ~cd ~/.config/nix-secrets~
3. ~git init && git add .~
...
1. Checkout this repo to ~$HOME/.config/nixos-config~
...
**** Using secrets
If you are going to use agenix secrets, add to your ~$HOME/.config/nix-secrets/flake.nix~ identity paths under the corresponding profiles to the ssh key path you want
