# Generated by Powerlevel10k configuration wizard on 2021-03-16 at 17:11 EDT.
...
() {
  ...
  typeset -g POWERLEVEL9K_LEFT_PROMPT_ELEMENTS=(
    ...
    ssh                 # os identifier
    aws2                     # aws profile (https://docs.aws.amazon.com/cli/latest/userguide/cli-configure-profiles.html)
  ...
  function prompt_ssh() {
      ...
      if [[ -z "$SSH_CONNECTION" ]]; then
          ...
          bkg=$(echo $(($((0x$(cksum <<<"$HOST" | cut -f 1 -d ' ')0))%230+1)))
          p10k segment -b $bkg -f 000 -t "@$HOST"
      fi
  }

  function prompt_aws2() {
      local tmp_showcond="$AWS_PROFILE"
      if [[ -z "$AWS_PROFILE" || "$SHOW_AWS_PROMPT" = false ]]; then
          tmp_showcond=""
      fi
      local tmp_bg='green'
      local tmp_fg='black'
      ...
      p10k segment -b $tmp_bg -f $tmp_fg  -t "$AWS_PROFILE" -c "${tmp_showcond}"
  }

  function prompt_time2() {
      PRINTTIME=$(date "+%I:%M")
      p10k segment -b 8 -f black -t "$PRINTTIME"
  }

  function prompt_prompt_post_look() {
      PRINTTIME=$(date "+%I:%M")
      p10k segment -b 22 -f gray -t "$PRINTTIME ❯"
  }
  function prompt_blank() {
      p10k segment -b black -t " "
  }


  function p10k-on-pre-prompt() {
      p10k display '<->|2/(left_frame|right_frame)'=show
  }
  function p10k-on-post-prompt() {
      p10k display '1|2/(right|right_frame|left_frame)'=show '2/left'=show
  }
  ...
  function instant_prompt_prompt_post_look() {
      # Since prompt_example always makes the same `p10k segment` calls, we can call it from
      # instant_prompt_example. This will give us the same `example` prompt segment in the instant
      # and regular prompts.
      prompt_prompt_post_look
  }
