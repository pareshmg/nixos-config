...
(use-package! ai-code
  :defer t
  :init
  ;; Enable global-auto-revert early if you want it to apply to all files
  (setq auto-revert-interval 1)
  (global-auto-revert-mode 1)
  :config
  ;; Set the backend (codex, gemini, etc.)
  (ai-code-set-backend 'claude-code-ide.el)

  ;; Bind the main menu. Doom usually prefers 'map!' for keybindings,
  ;; but global-set-key works too.
  ;; (map! :leader
  ;;       :desc "AI Code Mennu" "r" #'ai-code-menu)
  ;; (global-set-key (kbd "M-m r") #'ai-code-menu)
  ;; Or if you prefer your exact C-c a binding:

  ;; Setup Magit integration
  (after! magit
    (ai-code-magit-setup-transients)))


(map! "M-m r" #'ai-code-menu)
...
(after! breadcrumb
  ...
  (setq breadcrumb-header-line-format '("" breadcrumb-imenu-crumbs))
  (custom-set-faces!
    '(breadcrumb-face :foreground "yellow" :weight bold)
    '(header-line :background "#333333" :inherit mode-line)
    ))
